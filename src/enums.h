...
//
// Include in main.cpp and any module that touches events, state, or actions.
...
typedef enum
{
    ...
    // --- User button (if you derive from LTC/button line or separate input) ---
    EV_BTN_SHORT_PRESS,
    EV_BTN_LONG_PRESS,

    // --- DVR LED classifier output updates ---
    EV_DVR_LED_PATTERN_CHANGED,   // payload: dvr_led_pattern_t new_pattern

    // --- Battery sampling updates ---
    EV_BAT_STATE_CHANGED,         // payload: battery_state_t new_bat_state
...
typedef enum
{
    ...
    SRC_DVR_LED,
    SRC_BATTERY,
    SRC_FSM          // FSM can synthesize internal events if you choose
} event_source_t;
...
// Transition reason is separate from error_code_t so you can audit *normal*
// transitions cleanly.
typedef enum
{
    TR_NONE = 0,
...
typedef enum
{
    ...
    // --- Feedback ---
    ACT_BEEP,               // param: beep_pattern_t
    ACT_LED_PATTERN,        // param: led_pattern_t
    ...
    ACT_LTC_KILL_DEASSERT,  // normally you keep deasserted; included for completeness

    // --- Housekeeping / safety ---
    ACT_CLEAR_PENDING,      // clear event flags / reset classifier buffers
